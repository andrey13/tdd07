...
>[!NOTE]
**Примечание**. Существуют также другие режимы проверки, такие как never(), atLeast(),  atMost()  и  другие  средства  сопоставления  аргументов,  такие  как eq(), same(), any(), которые вы можете использовать в зависимости от вашего теста.
...
>[!NOTE]
Примечание.  Здесь  вы  можете  использовать  специальный сопоставитель строк, что сделает тест более надежным.
...
1.  Вы  собираетесь  сохранить  партитуру  в  CocktailsRepository  с  помощью SharedPreferences,  поэтому  вам  нужно  смоделировать  эту  зависимость  и  дать указание возвращать макет редактора всякий раз, когда запрашивается редактор. 
...
Вы  также  захотите  иметь  возможность  читать  рекорды  из  репозитория.  Для начала добавьте следующий тест:
...
Давайте  высушим  это,  рефакторинг  ваших  RepositoryUnitTests,  чтобы  он выглядел следующим образом:
...
class RepositoryUnitTests { 
	...
	@Before fun setup() { 
		api = mock() sharedPreferences = mock() 
...
Предположим, вы хотите сохранить рекорд только в том случае, если он выше ранее сохраненного рекорда. Для этого вы хотите начать с добавления следующего теста в свой класс RepositoryUnitTests:
...
В  этом  тесте  вы  заглушаете  метод  getHighScore(),  но  вам  также  нужно  вызвать реальный метод saveHighScore() для того же объекта, который является реальным объектом, CocktailsRepositoryImpl. Для этого вам нужен шпион, а не мошенник. Использование шпиона позволит вам вызывать методы реального объекта, а также отслеживать каждое взаимодействие, как если бы вы делали это с макетом. При настройке  шпионов  вам  нужно  использовать  doReturn/whenever/method  для заглушки метода. Попробуйте запустить тест, и вы увидите, что он не работает.
